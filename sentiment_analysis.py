...
# Example pipeline
pipeline = Pipeline([
    ('tfidf', TfidfVectorizer()),  # Combines tokenization and TF-IDF transformation
    ('classifier', SVC())
])
...
# Input from user
ip = st.text_input("Enter a review:")
...
if st.button("Predict"):
    if ip:
        try:
            # Make prediction
            output = model.predict([ip])
            st.write("Sentiment:", "Positive" if output[0] == 1 else "Negative")
...
pipeline = Pipeline([
    ('tfidf', TfidfVectorizer()),  # Vectorizer (learned from data)
    ('classifier', SVC())  # Classifier (e.g., Support Vector Machine)
])

# Fit the pipeline with training data
X_train = ["This is the first review", "This is the second review"]
...
# Load the pipeline and use it for prediction
loaded_pipeline = joblib.load('model_pipeline.pkl')
prediction = loaded_pipeline.predict(["This is a test review"])
print(prediction)

text_model = Pipeline([('vect',TfidfVectorizer()),('model',SVC())])

# Ensure that y_train has the same length as x_train
y_train = df['sentiment'][:749].values  # Use corresponding labels for the first 749 rows of data
x_train = df['review'][:749].values  # The reviews from the first 749 rows
